...
void goodbye_screen (hashcat_ctx_t *hashcat_ctx, const time_t proc_start, const time_t proc_stop)
{
  ...
  const time_t elapsed = (proc_stop >= proc_start) ? proc_stop - proc_start : 0;

  struct tm elapsed_tm;

  struct tm *elapsed_tmp = gmtime_r (&elapsed, &elapsed_tm);

  char elapsed_buf[HCBUFSIZ_TINY]; memset (elapsed_buf, 0, sizeof (elapsed_buf));

  format_timer_display (elapsed_tmp, elapsed_buf, sizeof (elapsed_buf));
